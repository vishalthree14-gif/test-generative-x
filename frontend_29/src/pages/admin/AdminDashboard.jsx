...
import { createQuiz, getQuiz, getCampaigns, createCampaign, generateQuestion, searchWeb, youtubeSearch, generateComplete } from "../../api/quizApi";
...
const AdminDashboard = () => {
  ...
  const handleCreateQuiz = async (e) => {
    ...
    try {
      ...
      setQuizForm({ topic: "", time_duration: "", difficulty: "", generate_status });
  ...
  const handleGenerate = async (quizId) => {
    setGeneratingId(quizId);
    setMessage({ text: "", type: "" });

    try {
      await Promise.all([
        generateQuestion(quizId),
        searchWeb(quizId),
        youtubeSearch(quizId),
        generateComplete(quizId)
      ]);

      setQuizzes((prev) =>
        prev.map((q) =>
          q.quiz_id === quizId ? { ...q, generate_status: true } : q
        )
      );
      setMessage({ text: "Quiz generated successfully!", type: "success" });
    } catch (err) {
      setMessage({
        text: err.response?.data?.message || "Failed to generate quiz",
        type: "error",
      });
    } finally {
                ...
                <div className="quiz-grid">
                  {quizzes.map((quiz, index) => (
                    <QuizCard
                      ...
                      generating={generatingId === quiz.quiz_id}
                    />
                  ))}
                </div>
              )}
